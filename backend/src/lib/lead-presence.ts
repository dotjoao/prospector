...
export type WebsiteUrlType = DigitalPresence;
...
export function classifyWebsiteUrl(url: string): WebsiteUrlType {
...
export function isInstagramOnlyLead(lead: Pick<Lead, 'website' | 'websiteAnalysis'>): boolean {
  return getDigitalPresence(lead) === 'instagram';
}
...
export function matchesExactFilter(value: string, filter: string): boolean {
